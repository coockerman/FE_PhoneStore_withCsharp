...
export default {
  ...
  methods: {
        ...
        const total = this.orders
          .filter((order) => {
            const orderDate = new Date(
              order.thoiGianDatHang
            ).toLocaleDateString("vi-VN", { month: "numeric", day: "numeric" });
            return orderDate === day;
          })
          .reduce((sum, order) => {
            return sum + order.price; // Thêm từ khóa return ở đây
          }, 0);
        return total;
      });
    ...
    calculateAndSetChartDataBy30Day() {
      ...
      const totals = allDays.map((day) => {
        const total = this.orders
          ...
          .reduce((sum, order) => {return sum + order.price}, 0);
        return total;
      });
      const defaultValues = Array(allDays.length).fill(0);
    ...
    calculateAndSetChartDataBy12Months() {
      ...
      this.data12month = totals.length > 0 ? totals.map(Number) : defaultValues;
    },
    getLabelByDay(day) {
      const labels = [];
